...
def test_index_post_request(client):
    ...
    with client as c:
        ...
        assert "1たす2は？" in decoded_data, "ユーザーの入力がHTML内に表示されるべきです。"
...
def test_memory_persistence(client):
    ...
    with client as c:
        ...
        assert any("1たす2は？" in msg['text'] for msg in messages if msg['class'] == 'user-message'), "メモリに最初のユーザーメッセージが保存されるべきです。"
...
def test_template_rendering(client):
    ...
    with client as c:
        ...
        assert "1たす2は？" in decoded_data, "テンプレートでユーザーの入力がレンダリングされるべきです。"
