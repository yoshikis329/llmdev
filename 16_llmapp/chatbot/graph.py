...
# ===== グラフを実行する関数 =====
def stream_graph_update(graph: StateGraph, user_message: str, thread_id):
    """
    ユーザからのメッセージをもとに、グラフを実行し、チャットボットの応答をストリーミングします。
    """
...
def get_bot_response(user_message, memory, thread_id):
    ...
    # グラフを実行してボットの応答を取得
    return stream_graph_update(graph, user_message, thread_id)
